 ...
 * @LastEditTime : 2020-01-13 14:31:42
 */
#include "sequence.h"
...
int SequenceExcuteWorker::runFunc(int row)
{
    ...
    // 
    qDebug()<<model->item(row, 1)->data();

    if (0 == model->item(row, 1))
    {
        Package::packOperateMulti(globalData->sendId, value, NODE_NUM, PROTOCOL_TYPE_SPD);
    }
    else if (1 == model->item(row, 1)->data())
    {
        for (int i = 0; i < NODE_NUM; i++) {
            value[i] += refValue[i];
            double realVal = 0;
...
void SequenceExcuteWorker::execSeqEvent()
{
    ...
    { //init row
        time.start();
        groupCnt = 1;
        listHeadBak = model->rowCount();
        listTail = model->rowCount() - 1;
 ...
 * @param model 返回读取的model的指针
 * @return: 状态: -1 失败; 0成功
 */
int importCSV(QString fileName, QStandardItemModel* model)
{
    QFile file(fileName);
    ...
    // 头两行为空
    else if (qlist[0].length() == 0 && qlist[1].length() == 0){
        file.close();
        return -1;
    }
    // 第一行不是'ref'
