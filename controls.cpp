/*
 * @Author: xingzhang.Wu 
 * @Date: 2019-09-29 09:58:00 
 * @Last Modified by:   xingzhang.Wu 
 * @Last Modified time: 2019-09-29 09:58:00 
 */
#include "controls.h"
...
*@date          20190731
**/
Controls::Controls(QWidget *parent) : QDialog(parent),
                                      currentNode(-1)
{
}
...
void Controls::speedValueChanged()
{
    ...
    {
        for (int i = 0; i < speedSpinBox.length() / 3; i++)
        { //4 parts
            if ((i * 3) < NODE_NUM && (Spin == findChild<QDoubleSpinBox *>(speedSpinBox[i * 3]) || Slider == findChild<DoubleSlider *>(speedSlider[i * 3])))
            {
                readyToSendCanData[i * 3].speed =
                    findChild<QDoubleSpinBox *>(speedSpinBox[i * 3])->text().toDouble();
            }
            else if ((i * 3) + 1 < NODE_NUM && (Spin == findChild<QDoubleSpinBox *>(speedSpinBox[(i * 3) + 1]) || Slider == findChild<DoubleSlider *>(speedSlider[(i * 3) + 1])))
            {
                readyToSendCanData[(i * 3) + 1].speed =
                    findChild<QDoubleSpinBox *>(speedSpinBox[(i * 3) + 1])->text().toDouble();
            }
            else if ((i * 3) + 2 < NODE_NUM && (Spin == findChild<QDoubleSpinBox *>(speedSpinBox[(i * 3) + 2]) || Slider == findChild<DoubleSlider *>(speedSlider[(i * 3) + 2])))
            {
...
void Controls::posValueChanged()
{
    ...
    {
        for (int i = 0; i < positionSpinBox.length() / 3; i++)
        { //4 parts
            if ((i * 3) < NODE_NUM && (Spin == findChild<QDoubleSpinBox *>(positionSpinBox[i * 3]) || Slider == findChild<DoubleSlider *>(positionSlider[i * 3])))
            {
                readyToSendCanData[i * 3].position =
                    findChild<QDoubleSpinBox *>(positionSpinBox[i * 3])->text().toDouble();
            }
            else if ((i * 3) + 1 < NODE_NUM && (Spin == findChild<QDoubleSpinBox *>(positionSpinBox[(i * 3) + 1]) || Slider == findChild<DoubleSlider *>(positionSlider[(i * 3) + 1])))
            {
                readyToSendCanData[(i * 3) + 1].position =
                    findChild<QDoubleSpinBox *>(positionSpinBox[(i * 3) + 1])->text().toDouble();
            }
            else if ((i * 3) + 2 < NODE_NUM && (Spin == findChild<QDoubleSpinBox *>(positionSpinBox[(i * 3) + 2]) || Slider == findChild<DoubleSlider *>(positionSlider[(i * 3) + 2])))
            {
...
void Controls::setPosButtonClicked()
{
    ...
    {
        for (int i = 0; i < positionSetButton.length() / 3; i++)
        { //4 parts
...
void Controls::setSpeedButtonClicked()
{
    ...
    {
        for (int i = 0; i < speedSetButton.length() / 3; i++)
        { //4 parts
